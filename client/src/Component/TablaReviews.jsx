...
function TablaReviews(props) {
  ...
  function borrarReview(idReview){
      axios.delete(`http://localhost:4000/products/${idReview}/review`).then((response) => {
        props.removeReview(idReview)
      })}
                    ...
                    {props.reviews.reviews.filter(elem => elem.productId == idP) &&
                      props.reviews.reviews.filter(elem => elem.productId == idP).map((row) => (
                        <TableRow key={row.name}>
                          <Grid item>
                            <ButtonGroup>
                              {props.user.setUser && props.user.setUser.id == row.userId &&(<IconButton aria-label="delete" className={classes.margin} onClick={e => borrarReview(row.id)}>
...
const mapDispatchToProps = dispatch => ({
  ...
  removeReview: r => dispatch(removeReview(r)),
})
